...
namespace MP3Thumbnails
{
    public partial class _Default : System.Web.UI.Page
    {
        ...
        private CloudBlobContainer getMP3Container()
        {
            return _blobStorageService.getCloudBlobContainer();
        }

        private CloudQueue getMP3MakerQueue()
        {
        ...
        protected void submitButton_Click(object sender, EventArgs e)
        {
            if (upload.HasFile)
            {
                ...
                String path = "mp3Files/" + name;

                var blob = getMP3Container().GetBlockBlobReference(path);
                ...
                // for easier display. 
                getMP3MakerQueue().AddMessage(new CloudQueueMessage(System.Text.Encoding.UTF8.GetBytes(name)));

                System.Diagnostics.Trace.WriteLine(String.Format("*** WebRole: Enqueued '{0}'", path));
            }
        }

        //Get MP3 title to be used in Page PreRender methos
        private object getMP3Title(Uri uri)
        {
            CloudBlockBlob cbb = new CloudBlockBlob(uri);
            cbb.FetchAttributes();
            return cbb.Metadata["Title"];
        }
        ...
        protected void Page_PreRender(object sender, EventArgs e)
        {
            try
            {
                ...
                MP3SamplesDisplayControl.DataSource = from o in getMP3Container().GetDirectoryReference("samples").ListBlobs()
                                                     select new { Url = o.Uri, Title = getMP3Title(o.Uri) };

                // Tell the list view to bind to its data source, thereby
                // showing 
                MP3SamplesDisplayControl.DataBind();
            }
            catch (Exception)
            {
            }
